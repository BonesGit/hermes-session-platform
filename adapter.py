...
Bridge: <plugin>/bridge/session-bridge.mjs
...
def resolve_bridge_script() -> Path:
    """Return the absolute path to session-bridge.mjs shipped with this plugin."""
...
class SessionAdapter(BasePlatformAdapter):
    ...
    async def _handle_message_event(self, msg_data: dict) -> None:
        ...
        # Group messages: adapter-enforced allowlists + @mention gating
        if is_group:
            sender_id = msg_data.get("source") or ""
            if not is_group_message_allowed(
                chat_id=conversation_id,
                sender_id=sender_id,
                group_allowed_chats=_parse_csv_env("SESSION_GROUP_ALLOWED_CHATS"),
                group_allowed_users=_parse_csv_env("SESSION_GROUP_ALLOWED_USERS"),
            ):
                logger.debug(
                    "Session: dropping group message (allowlist) chat=%s sender=%s",
                    conversation_id[:12],
                    (sender_id or "")[:12],
                )
                return
...
# ---------------------------------------------------------------------------
# Pure helpers (group allowlists, env parsing)
# ---------------------------------------------------------------------------

def _parse_csv_env(name: str) -> list:
...
def is_group_message_allowed(
    ...
    """Enforce SESSION_GROUP_ALLOWED_* without core authz_mixin maps.

    Empty allowlists → allow (mention gating still applies separately).
    Non-empty GROUP_ALLOWED_CHATS → chat_id must be listed.
    Non-empty GROUP_ALLOWED_USERS → sender_id must be listed.
    """
    chats = group_allowed_chats if group_allowed_chats is not None else []
    users = group_allowed_users if group_allowed_users is not None else []
...
def check_requirements() -> bool:
    """Registry-facing alias for check_session_requirements()."""
    return check_session_requirements()


def validate_config(config) -> bool:
    extra = getattr(config, "extra", {}) or {}
    bot_id = os.getenv("SESSION_BOT_ID") or extra.get("bot_id") or ""
    return bool(str(bot_id).strip())
...
def _env_enablement():
    ...
    bot_id = (os.getenv("SESSION_BOT_ID") or "").strip()
    if not bot_id:
        return None
    try:
    ...
    seed = {
        ...
        "data_path": os.getenv("SESSION_DATA_PATH", str(home / "session-data")),
    ...
    session_home = (os.getenv("SESSION_HOME_CHANNEL") or "").strip()
    if session_home:
        seed["home_channel"] = {
...
async def _standalone_send(
    ...
    *,
    ...
    """Out-of-process Session delivery via the local bridge HTTP API.

    Requires the bridge (usually spawned by the live gateway adapter) to be
    listening on bridge_port. Cron deliver=session works when the gateway is up.
    """
    extra = getattr(pconfig, "extra", {}) or {}
    port = extra.get("bridge_port") or os.getenv("SESSION_BRIDGE_PORT") or "8095"
    url = f"http://127.0.0.1:{port}/send"
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            text = message or ""
            if text.strip():
                resp = await client.post(url, json={"to": chat_id, "body": text})
            ...
            for item in media:
                ...
                if resp.status_code >= 400:
                    logger.warning(
                        "Session standalone media send failed: %s", resp.text
                    )
...
def interactive_setup() -> None:
    """Interactive `hermes gateway setup` flow for Session."""
    import json
    import shutil
    ...
    if existing_bot_id:
        print_info(
            f"Session: already configured (bot ID: {existing_bot_id[:12]}...)"
        )
    ...
    if not bridge_script.exists():
        ...
        print_info("Reinstall the session-platform plugin so bridge/ is present.")
        return
    ...
    if npm_result.returncode != 0:
        ...
        print_error(f"npm install failed: {err}")
        print_info(
            "See https://github.com/BonesGit/session-desktop-library/"
            "blob/dev-library/CONTRIBUTING.md"
        )
        return
    print_success("Session bridge dependencies installed")

    data_path = get_env_value("SESSION_DATA_PATH") or str(
        get_hermes_home() / "session-data"
    )
    ...
    if db_bot_id:
        print_warning(
            f"An account is already registered in the data dir (bot ID: {db_bot_id})."
        )
        ...
        if env_bot_id and env_bot_id != db_bot_id:
            ...
            print_warning(
                "The .env value will be updated to match the registered account."
            )
    ...
    has_mnemonic = prompt_yes_no(
        "Do you have an existing Session mnemonic to restore?", False
    )
    mnemonic = None
    if has_mnemonic:
        ...
        if db_bot_id:
            print_warning(
                f"Data directory already has account {db_bot_id} registered."
            )
            print_warning(
                "Wipe the data directory before restoring a different account:"
            )
            if sys.platform == "win32":
    ...
    bot_name = prompt(
        "Bot display name",
        default=get_env_value("SESSION_BOT_NAME") or "Hermes",
    )
    print_info("Enter YOUR Session ID (Session app → Settings).")
    ...
    recipient_id = prompt(
        "Your Session ID", default=existing_recipient or None
    )
    if not recipient_id:
    ...
    try:
        result = subprocess.run(
            [node, "--no-deprecation", str(bridge_script), "--setup"],
            env=env,
            capture_output=True,
            text=True,
            timeout=90,
        ...
        print_error(
            f"Account {action} timed out. Check Node.js and try again."
        )
        return

    if result.returncode != 0:
        err = (result.stderr or result.stdout or "").strip()
        print_error(f"Bridge setup failed: {err}")
        return
    ...
    if status == "restored":
        ...
        print_success(
            f"Existing account found! Bot Session ID: {bot_session_id}"
        )
    else:
        print_success(f"Account created! Bot Session ID: {bot_session_id}")
    print_info("Share this Session ID with contacts.")
    if mnemonic:
        print_info("Mnemonic saved to .env — back it up somewhere safe.")
        print_info(
            "The gateway does not need it at runtime; you may delete it from .env."
        )
    print_success("Session configured!")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        name="session",
        label="Session",
        adapter_factory=lambda cfg: SessionAdapter(cfg),
        check_fn=check_requirements,
        validate_config=validate_config,
        is_connected=is_connected,
        required_env=["SESSION_BOT_ID"],
        install_hint=(
            "Requires Node.js >= 24.12.0 and `npm install` in this plugin's "
            "bridge/ directory. Enable with: "
            "hermes plugins enable session-platform && hermes gateway setup"
        ),
        setup_fn=interactive_setup,
        env_enablement_fn=_env_enablement,
        cron_deliver_env_var="SESSION_HOME_CHANNEL",
        standalone_sender_fn=_standalone_send,
        allowed_users_env="SESSION_ALLOWED_USERS",
        allow_all_env="SESSION_ALLOW_ALL_USERS",
        max_message_length=MAX_MESSAGE_LENGTH,
        emoji="🔒️",
        pii_safe=False,
        ...
        platform_hint=(
            ...
            "Group IDs start with '03', DMs with '05'. "
            "Supports attachments, voice, images, disappearing messages, "
            "and emoji reactions."
        ),
    )
